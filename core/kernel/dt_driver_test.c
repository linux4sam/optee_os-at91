...
TEE_Result dt_driver_test_status(void)
{
	...
	if (IS_ENABLED(CFG_DRIVERS_GPIOS) &&
		...
		EMSG("Gpios controllers probing test failed");
		res = TEE_ERROR_GENERIC;
	}
					 ...
					 const void *compat_data __unused)
{
	...
	if (IS_ENABLED(CFG_DRIVERS_GPIO)) {
		...
		res = gpio_dt_get_by_index(fdt, node, 0, "foo", &gpio);
		if (res)
			goto err_probe;
		...
		res = gpio_dt_get_by_index(fdt, node, 1, "foo", &gpio);
		if (res)
			goto err_probe;
					...
					void *data, TEE_Result *res)
{
	...
	gpio = gpio_get_dt_two_cells(a, res);
	if (*res)
		return NULL;
	...
	case DT_TEST_GPIO0_PIN:
		...
		if (gpio->dt_flags != gpios[0].flags) {
			EMSG("Unexpected dt_flags %u", gpio->dt_flags);
			*res = TEE_ERROR_GENERIC;
			return NULL;
		}
		break;
