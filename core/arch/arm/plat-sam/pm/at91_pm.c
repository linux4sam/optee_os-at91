...
static struct at91_pm_data soc_pm = {0};

/* Backup canary */
static unsigned long canary = 0xA5A5A5A5;

/* Backup mode informations used by at91bootstrap */
...
static int at91_pm_config_ws_ulp1(bool set)
{
	...
	for (src = 0; src < ARRAY_SIZE(sama5d2_ws_ids); src++) {
		...
		node = fdt_node_offset_by_compatible(soc_pm.fdt, -1, wsrc->compatible);
		while (node >= 0) {
...
next_node:
			node = fdt_node_offset_by_compatible(soc_pm.fdt, node, wsrc->compatible);
		}
	}
	...
	return mode ? TEE_SUCCESS : TEE_ERROR_BAD_STATE;
}
...
TEE_Result at91_pm_suspend(uintptr_t entry, struct sm_nsec_ctx *nsec)
{
	 ...
	 */
	if (res == 0 && soc_pm.mode == AT91_PM_BACKUP)
		nsec->mon_lr = entry;

	DMSG("Exiting suspend mode %d correctly", soc_pm.mode);
...
static TEE_Result at91_pm_sram_init(const void *fdt)
{
	int node = -1;
	size_t size = 0;
	static paddr_t at91_suspend_sram_pbase = 0;
	size_t suspend_sz = at91_pm_suspend_in_sram_sz;

	assert(at91_pm_suspend_in_sram_sz <= SZ_64K);
...
static TEE_Result sama5d2_pm_init_all(const void *fdt, vaddr_t shdwc)
{
	...
	COMPILE_TIME_ASSERT(CFG_AT91_PM_DEFAULT_SUSPEND_MODE >= AT91_PM_STANDBY &&
			    CFG_AT91_PM_DEFAULT_SUSPEND_MODE <= AT91_PM_BACKUP);
	...
	soc_pm.mode = CFG_AT91_PM_DEFAULT_SUSPEND_MODE;
...
TEE_Result sama5d2_pm_init(const void *fdt, vaddr_t shdwc)
{
	if (sama5d2_pm_init_all(fdt, shdwc))
		panic("Failed to setup PM for at91");
