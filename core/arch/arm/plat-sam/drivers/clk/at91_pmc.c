...
static void pmc_suspend(void)
{
	...
	for (i = 0; registered_ids[i]; i++) {
		...
		pmc_cache.pcr[registered_ids[i]] = io_read32(pmc_base + AT91_PMC_PCR);
	}
...
static void pmc_resume(void)
{
	...
	if (pmc_cache.mckr != tmp)
		EMSG("MCKR was not configured properly by the firmware\n");
	tmp = io_read32(pmc_base + AT91_CKGR_PLLAR);
	if (pmc_cache.pllar != tmp)
		EMSG("PLLAR was not configured properly by the firmware\n");
...
void pmc_register_pm(void)
{
	 ...
	 */
	register_pm_core_service_cb(pmc_pm, NULL);
}
