...
static inline bool clk_utmi_ready(vaddr_t pmc_base)
{
	unsigned int status = io_read32(pmc_base + AT91_PMC_SR);
...
static int clk_utmi_is_enabled(struct clk *clk)
{
	...
	return clk_utmi_ready(utmi->pmc_base);
}
...
static void clk_utmi_disable(struct clk *clk)
{
	...
	io_clrsetbits32(utmi->pmc_base + AT91_CKGR_UCKR, AT91_PMC_UPLLEN, 0);
}
