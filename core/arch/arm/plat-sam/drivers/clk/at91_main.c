...
static int pmc_main_rc_osc_enable(struct clk *clk)
{
	struct main_rc_osc *osc = clk->priv;
	uint32_t mor = io_read32(osc->base + AT91_CKGR_MOR);
	uint64_t timeout = timeout_init_us(OSC_READY_TIMEOUT_US);

	/* Enable the oscillator if not */
	if (!(mor & AT91_PMC_MOSCRCEN)) {
		mor &= ~(MOR_KEY_MASK | AT91_PMC_MOSCRCEN);
...
static unsigned long pmc_main_rc_osc_get_rate(struct clk *clk,
 ...
 * Main Oscillator
 */
static inline bool pmc_main_osc_ready(struct pmc_data *pmc)
{
...
static int pmc_main_osc_enable(struct clk *clk)
{
...
static int pmc_main_osc_is_enabled(struct clk *clk)
{
	struct pmc_data *pmc = clk->priv;
	uint32_t mor = io_read32(pmc->base + AT91_CKGR_MOR);
	uint32_t status;
	...
	status = io_read32(pmc->base + AT91_PMC_SR);

	return (status & AT91_PMC_MOSCS) && clk_main_parent_select(mor);
}

static const struct clk_ops pmc_main_osc_clk_ops = {
	.enable = pmc_main_osc_enable,
	.disable = pmc_main_osc_disable,
	.is_enabled = pmc_main_osc_is_enabled,
};
				  ...
				  struct clk *parent, bool bypass)
{
	...
	if (bypass)
		io_clrsetbits32(pmc->base + AT91_CKGR_MOR, MOR_KEY_MASK |
				AT91_PMC_OSCBYPASS,
...
static int clk_main_probe_frequency(vaddr_t base)
{
	uint64_t timeout = timeout_init_us(OSC_READY_TIMEOUT_US);
	unsigned int mcfr;
...
static inline bool clk_sam9x5_main_ready(vaddr_t base)
{
...
static int clk_sam9x5_main_is_enabled(struct clk *clk)
{
	...
	return clk_sam9x5_main_ready(pmc->base);
}
...
static int clk_sam9x5_main_set_parent(struct clk *clk, uint8_t index)
{
	struct pmc_data *pmc = clk->priv;
	unsigned int tmp;
...
static uint8_t clk_sam9x5_main_get_parent(struct clk *clk)
{
	struct pmc_data *pmc = clk->priv;
