...
static void pmc_setup(void *fdt, int nodeoffset)
{
	size_t size;
	vaddr_t base;
	unsigned int i;
	...
	slow_clk = clk_of_get_by_name(fdt, nodeoffset, "slow_clk");
	if (!slow_clk)
		panic();

	main_xtal_clk = clk_of_get_by_name(fdt, nodeoffset, "main_xtal");
	if (!main_xtal_clk)
		panic();
	...
	clk_scmi_set_ids(main_clk, 0, AT91_SCMI_CLK_CORE_MAIN);
	...
	clk_scmi_set_ids(plladivck, 0, AT91_SCMI_CLK_CORE_PLLACK);
	...
	clk_scmi_set_ids(audiopll_pmcck, 0, AT91_SCMI_CLK_CORE_AUDIOPLLCK);
	...
	clk_scmi_set_ids(utmi_clk, 0, AT91_SCMI_CLK_CORE_UTMI);
	...
	clk_scmi_set_ids(mckdivck, 0, AT91_SCMI_CLK_CORE_MCK);
	...
	clk_scmi_set_ids(h32mxck, 0, AT91_SCMI_CLK_CORE_MCK2);
	...
	for (i = 0; i < ARRAY_SIZE(sama5d2_progck); i++) {
		...
		clk_scmi_set_ids(clk, 0, sam_clk->scmi_id);
		pmc_clk = &pmc->pchws[i];
		pmc_clk->clk = clk;
	...
	clk_scmi_set_ids(clk, 0, AT91_SCMI_CLK_CORE_I2S0_MUX);
	...
	clk_scmi_set_ids(clk, 0, AT91_SCMI_CLK_CORE_I2S1_MUX);
	pmc->chws[PMC_I2S1_MUX].clk = clk;
	pmc->chws[PMC_I2S1_MUX].id = PMC_I2S1_MUX;

	clk_of_register_clk_provider(nodeoffset, clk_of_pmc_get, pmc);
}

CLK_OF_DECLARE(sama5d2_clk, "atmel,sama5d2-pmc", pmc_setup);
