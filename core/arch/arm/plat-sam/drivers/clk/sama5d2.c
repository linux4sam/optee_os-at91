...
static void pmc_setup(void *fdt, int nodeoffset)
{
	...
	clk_scmi_match(main_clk, 0, AT91_SCMI_CLK_CORE_MAIN);
	...
	clk_scmi_match(plladivck, 0, AT91_SCMI_CLK_CORE_PLLACK);
	...
	clk_scmi_match(audiopll_pmcck, 0, AT91_SCMI_CLK_CORE_AUDIOPLLCK);
	...
	clk_scmi_match(utmi_clk, 0, AT91_SCMI_CLK_CORE_UTMI);
	...
	clk_scmi_match(mckdivck, 0, AT91_SCMI_CLK_CORE_MCK);
	...
	clk_scmi_match(h32mxck, 0, AT91_SCMI_CLK_CORE_MCK2);
	...
	for (i = 0; i < ARRAY_SIZE(sama5d2_progck); i++) {
		...
		clk_scmi_match(clk, 0, sam_clk->scmi_id);
		pmc_clk = &pmc->pchws[i];
		pmc_clk->clk = clk;
	...
	clk_scmi_match(clk, 0, AT91_SCMI_CLK_CORE_I2S0_MUX);
	...
	clk_scmi_match(clk, 0, AT91_SCMI_CLK_CORE_I2S1_MUX);
	pmc->chws[PMC_I2S1_MUX].clk = clk;
	pmc->chws[PMC_I2S1_MUX].id = PMC_I2S1_MUX;

	clk_of_register_clk_provider(fdt, nodeoffset, clk_of_pmc_get, pmc);
}
