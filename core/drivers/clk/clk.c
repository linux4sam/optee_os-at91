...
static TEE_Result clk_set_parent_no_lock(struct clk *clk, size_t pidx)
{
	TEE_Result res = TEE_ERROR_GENERIC;
...
TEE_Result clk_set_parent(struct clk *clk, size_t pidx)
{
	uint32_t exceptions = 0;
	TEE_Result res = TEE_ERROR_GENERIC;
	...
	res = clk_set_parent_no_lock(clk, pidx);
out:
	cpu_spin_unlock_xrestore(&clk_lock, exceptions);
			       ...
			       unsigned long *rates, size_t *nb_elts)
{
	...
	cpu_spin_lock(&clk_lock);
	res = clk->ops->get_rates_array(clk, start_index, rates, nb_elts);
