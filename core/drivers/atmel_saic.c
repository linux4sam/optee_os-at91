...
static void saic_configure_it(size_t it, int src_type, int priority)
{
	saic_select_it(it);
...
static uint32_t saic_get_src_type(int dt_level, int it, uint32_t *src_type)
{
	switch (dt_level) {
		     ...
		     uint32_t type, uint32_t prio)
{
	...
	uint32_t res = TEE_ERROR_GENERIC;

	if (it >= saic.nr_irqs)
		panic();

	res = saic_get_src_type(type, it, &src_type);
	if (res)
...
static int saic_dt_get_irq(const uint32_t *properties, int len,
			   uint32_t *type, uint32_t *prio)
{
	uint32_t res = DT_INFO_INVALID_INTERRUPT;
	...
	res = saic_get_src_type(irq_type, it, &src_type);
	if (res)
		return res;
...
static void saic_clear_aicredir(void)
{
	...
	/*
	 * We explicitely don't want to redirect secure interrupts to non secure
...
static void saic_register_pm(void)
{
	register_pm_core_service_cb(saic_pm, NULL);
}
#else
static void saic_register_pm(void)
{
